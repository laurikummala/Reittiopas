...
// Reitin tietojen päivitys ei toimi, palaan asiaan joskus!!!
// Päivitä käyttäjän tietyn reitin tiedot, parametreina: id-numero, reittiData ja token
const paivitaReitti = async (reittiId, reittiData, token) => {
  const config = {
    headers: {
      Authorization: `Bearer ${token}`
    }
  ...
  const response = await axios.put(API_URL + reittiId, reittiData, config)
