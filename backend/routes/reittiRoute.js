...
router.route('/').get(protect, getReitit).post(protect, setReitti)
router.route('/:id').delete(protect, deleteReitti).put(protect, updateReitti)
...
// 	// POISTETTU 4 TUTORIAALIN OHJEEN MUKAAN const user = await User.findById(req.user.id)
...
// 	// POISTETTU 4 TUTORIAALIN OHJEEN MUKAAN const user = await Reitti.findById(req.user.id)
